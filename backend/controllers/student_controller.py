...
class StudentController:
    ...
    def get_all(db: Session, owner_id: str) -> list[StudentDB]:
        return db.query(StudentDB).filter(StudentDB.owner_id == owner_id).order_by(StudentDB.id.desc()).all()

    @staticmethod
    def get_by_id(student_id: int, db: Session, owner_id: str) -> StudentDB:
        student = db.query(StudentDB).filter(StudentDB.id == student_id, StudentDB.owner_id == owner_id).first()
        if not student:
